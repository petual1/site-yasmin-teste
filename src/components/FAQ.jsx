...
const FAQ = () => {
  ...
  // -styles-
  const buttonWrapper = "group relative w-72 h-16 rounded-full shadow-lg bg-yasmin-olive overflow-hidden cursor-pointer hover:shadow-2xl hover:-translate-y-1 transition-all duration-300";
  ...
  const halfButton = "w-full h-1/2 flex items-center justify-center text-lg font-bold gap-2";

  // -jsx-
  return (
    <section id="duvidas" className="pt-20 md:pt-32 pb-10 bg-[#FCFDF5] relative overflow-hidden z-10 w-full max-w-[100vw]">
      ...
      <div 
        ref={sectionRef}
        className={`max-w-7xl mx-auto px-6 grid lg:grid-cols-12 gap-12 lg:gap-20 items-start relative z-10 transition-all duration-1000 ease-out transform ${
      ...
      >
        ...
        <div className="lg:col-span-4 lg:sticky lg:top-32 text-center lg:text-left">
          ...
          <h2 className="text-4xl md:text-5xl font-serif mb-6 leading-tight text-yasmin-olive">
            ...
            {faqs.map((item, idx) => (
              <div key={idx} className="group">
                <button
                  ...
                  className="w-full py-6 flex items-center justify-between text-left focus:outline-none group-hover:text-yasmin-rose transition-colors duration-300"
                >
                  <span className={`text-xl md:text-2xl font-serif ${openIndex === idx ? 'text-yasmin-rose' : 'text-yasmin-olive'}`}>
                    {item.question}
                  </span>
                ...
                >
                  <p className="text-gray-700 font-sans font-light leading-relaxed text-lg pr-4 md:pr-10 text-left">
                    {item.answer}
                  </p>
                </div>
              </div>
